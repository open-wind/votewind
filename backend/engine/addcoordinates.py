...
if __name__ == "__main__":
    ...
    with open('groups/england.csv', 'r', newline='', encoding='latin1') as csvfile:
        ...
        for row in reader: 
            if (row['Postcode'] != '') and (row['Longitude'] == ''):
                coordinates = postcode_to_latlng(row['Postcode'])
                if coordinates['longitude'] is not None:
                    row['Longitude'] = coordinates['longitude']
                    row['Latitude'] = coordinates['latitude']
            output_data.append(row)

    with open('groups/england_new.csv', "w", encoding='UTF-8') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=output_data[0].keys())
        writer.writeheader()
        for row in output_data: writer.writerow(row)
