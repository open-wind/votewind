...
if __name__ == "__main__":
    ...
    with open('groups/england.csv', 'r', newline='', encoding='latin1') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader: 
            if (row['Website URL'] != '') and (row['Postcode'] == '') and (row['Longitude'] == ''):
                website = row['Website URL']
                if 'facebook.com' in website: continue
                website_data = scrape_website(website)

                if (website_data['postcode'] is not None) and (website_data['postcode'] != ''):
                    print(row['Organisation Name'], website_data['postcode'])
