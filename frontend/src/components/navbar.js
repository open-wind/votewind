...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    let calculatedDomain = APP_BASE_URL;

    if (parts.length > 1) {
      if (parts[0] === 'mobile') {
        calculatedDomain = 'https://mobile.votewind.org';
      }
    }
  ...
  <nav className="fixed top-0 h-20 left-0 w-full z-50 bg-gradient-to-b from-white/80 to-white/0 text-black px-4 py-2 flex justify-between items-center pointer-events-none">
   ...
   <div className="absolute inset-x-0 top-0 h-14 flex items-center justify-between px-4 sm:px-6 font-bold pointer-events-auto">
      ...
      <div className="text-xl pl-1" style={{ textShadow: '0 0px 20px rgba(255,255,255,0)' }}>
        <a href={baseDomain}><span className="text-black font-light sm:font-light tracking-wide sm:tracking-normal pr-[1px]">VoteWind.org</span></a>
      </div>
      ...
      <div className="space-x-6 hidden pr-2 md:flex items-right text-sm text-black font-medium tracking-wider pl-1" style={{ textShadow: '0 0px 30px rgba(255,255,255,1)' }}>
        <a href={baseDomain} className="hover:underline">START</a>
        <a href={baseDomain + "/map"} className="hover:underline">MAP</a>
        <a href={baseDomain + "/leaderboard"} className="hover:underline">LEADERBOARD</a>
        <a href={CONTENT_BASE_URL + "/guide"} className="hover:underline">GUIDE</a>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 w-full bg-white font-light shadow-md flex flex-col items-start px-4 py-2 md:hidden z-50">
          <a href={baseDomain} className="py-2 w-full hover:underline">Home</a>
          <a href={baseDomain + "/map"} className="py-2 w-full hover:underline">Map</a>
          <a href={baseDomain + "/leaderboard"} className="py-2 w-full hover:underline">Leaderboard</a>
          <a href={CONTENT_BASE_URL + "/guide"} className="py-2 w-full hover:underline">Guide</a>
