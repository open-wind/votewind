...
import { Plus, Minus, Layers, ExternalLink, Wind } from 'lucide-react';
import { IoMdClose } from 'react-icons/io'; 
// import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
// import { faBinoculars } from '@fortawesome/free-solid-svg-icons';
import { UserGroupIcon } from '@heroicons/react/24/solid'; // or `/outline`
import ViewInArIcon from '@/components/ui/view-in-ar-icon';
const querystring = require('querystring');
import { Button } from "@/components/ui/button";
import { useIsMobile, isIOS, windspeed2Classname, windspeedToInterpolatedColor } from "@/components/functions/helpers"
...
export default function VoteWindMap({ longitude=null, latitude=null, zoom=null, type='', properties=null, bounds=null, style=null, turbineAtCentre=false }) {
    ...
    const showOrganisationsRef = useRef(showOrganisations);
    ...
    // This callback will be triggered by map.on('zoomend')
    const updateZoomStates = useCallback((mapInstance) => { 
        setCurrentZoom(mapInstance.getZoom());
        setMinZoom(mapInstance.getMinZoom());
        setMaxZoom(mapInstance.getMaxZoom());

        const maxBounds = mapInstance.getMaxBounds?.();
        const visibleBounds = mapInstance.getBounds?.();

        if (!maxBounds || !visibleBounds) {
            console.warn("Bounds not available yet");
            return;
        }

        const visibleNE = visibleBounds.getNorthEast();
        const visibleSW = visibleBounds.getSouthWest();
        const maxNE = maxBounds.getNorthEast();
        const maxSW = maxBounds.getSouthWest();

        const visibleWidth = visibleNE.lng - visibleSW.lng;
        const visibleHeight = visibleNE.lat - visibleSW.lat;
        const maxWidth = maxNE.lng - maxSW.lng;
        const maxHeight = maxNE.lat - maxSW.lat;

        if ((visibleWidth >= maxWidth) || (visibleHeight >= maxHeight)) {
            setMinZoom(mapInstance.getZoom());
        }
    }, []);
    ...
    const incorporateBaseDomain = (tileserver_baseurl, api_baseurl, json) => {
        ...
        for (const layer_id of LAYERS_ALLCONSTRAINTS) {
            newjson['sources'][layer_id] = {
                ...
                'attribution': 'Legacy browser application. Source data copyright of multiple organisations. For all data sources, see <a href="https://data.openwind.energy" target="_blank">data.openwind.energy</a>'
            }
    ...
    useEffect(() => {
        showOrganisationsRef.current = showOrganisations;
    }, [showOrganisations]);
    ...
    const toggleOrganisations = () => {
        ...
        if (!showOrganisations) {
            ...
            if (map.getZoom() >= MAP_MINZOOM_CONSTRAINTS) {
                setShowNearestOrganisations(true);
            }
        } else {
    ...
    useEffect(() => {
        ...
        if (turbinePosition.longitude && turbinePosition.latitude) retrieveTurbinePositionData();
    ...
    useEffect(() => {
        ...
        // If not idle, don't do anything else
        if (!isIdle) return;
        ...
        // If position exactly coincides with existing vote, select vote - which hides turbine marker
        const votefeatures = map.queryRenderedFeatures(screenPoint, { layers: ['votes-confirmed', 'votes-unconfirmed']});
        if ((votefeatures.length > 0) && (!votes)) {
            const turbineLongitude = turbinePosition.longitude.toFixed(5);
            const turbineLatitude = turbinePosition.latitude.toFixed(5);
            for(var i = 0; i < votefeatures.length; i++) {
    ...
    }, [turbinePosition, turbineAdded, isIdle]);
    ...
    const onLoad = () => {
        ...
        map.on('zoomend', () => updateZoomStates(map)); // Pass the map instance to the updater

        // Perform initial state update immediately after load
        updateZoomStates(map);
    ...
    const onMoveEnd = (e) => {
        ...
        checkZoom(mapRef?.current.getMap());
        setIsIdle(true);
    }
    ...
    const checkZoom = (map) => {
        ...
        if (showOrganisationsRef.current) setShowNearestOrganisations(showConstraintsNewState);
    ...
    const mapCentreOnSubstation = () => {
        ...
        if (map.getZoom() < MAP_SUBSTATION_ZOOM) {
            map.flyTo({center: {lng: substation.position.longitude, lat: substation.position.latitude}, animate: true, zoom: MAP_SUBSTATION_ZOOM, padding: {top: topPadding, bottom: turbineAdded ? window.innerHeight / 3 : 0}});
        } else {
            map.flyTo({center: {lng: substation.position.longitude, lat: substation.position.latitude}, animate: true, padding: {top: topPadding, bottom: turbineAdded ? window.innerHeight / 3 : 0}});
        }
        resetPadding(map);
    }
    ...
    const mapCentreOnTurbine = () => {
        ...
        if ((map.getZoom() < MAP_PLACE_ZOOM) && (!showViewshed)) {
            map.flyTo({center: {lng: turbinePosition.longitude, lat: turbinePosition.latitude}, zoom: MAP_PLACE_ZOOM, animate: true, padding: {top: topPadding, bottom: turbineAdded ? window.innerHeight / 3 : 0}});
        } else {
            map.flyTo({center: {lng: turbinePosition.longitude, lat: turbinePosition.latitude}, animate: true, padding: {top: topPadding, bottom: turbineAdded ? window.innerHeight / 3 : 0}});
        }
        resetPadding(map);
    }
    ...
    const mapCentreOnOrganisation = () => {
        ...
        if (map.getZoom() < MAP_PLACE_ZOOM) {
            map.flyTo({center: {lng: organisation.longitude, lat: organisation.latitude}, zoom: MAP_PLACE_ZOOM, animate: true, padding: {top: topPadding, bottom: (organisation !== null) ? window.innerHeight / 3 : 0}});
        } else {
            map.flyTo({center: {lng: organisation.longitude, lat: organisation.latitude}, animate: true, padding: {top: topPadding, bottom: (organisation !== null) ? window.innerHeight / 3 : 0}});
        }
        resetPadding(map);
    }
    ...
    <div className="min-h-screen flex flex-col justify-between" style={{ height: 'calc(var(--vh, 1vh) * 100)' }}>
        ...
        <div className="flex justify-center items-center w-screen">
            ...
            <div className="w-full h-full sm:h relative">
                ...
                {/* Vertical toolbar */}
                <div className="absolute left-4 sm:left-4 top-8-3rem sm-top-9-3rem z-40 select-none">
                    <div className="bg-gray-100 rounded-full shadow-lg p-2 sm:p-2 flex flex-col items-center space-y-2 sm:space-y-2">
                    ...
                    <TooltipProvider>
                        ...
                        <TooltipTrigger asChild>
                            ...
                            className={`w-8 h-8 sm:w-10 sm:h-10 p-1 ${(showWindspeeds) ? ("bg-blue-600") : ("bg-white")} text-blue-700 rounded-full shadow transition flex items-center justify-center noselect`}
                            >
                                {showWindspeeds ? (
                                    <Wind className="noselect w-6 h-6 text-white" style={{ filter: "drop-shadow(1px 1px 2px rgba(0,0,0,0.5))" }}/>
                                ) : (
                                    <div className="relative w-6 h-6 flex items-center justify-center rounded-full">
                                        <Wind className="noselect w-6 h-6 text-gray-400" strokeWidth={1.5} />
                                    </div>
                                )}

                                {showWindspeeds && positionWindspeed && (
                                <div style={{ backgroundColor: windspeedToInterpolatedColor(positionWindspeed) }} className={`absolute top-0 left-0 translate-x-8 sm:translate-x-10 -translate-y-2 min-w-7 pl-2 pr-2 h-6 border-2 sm:border-2 border-white rounded-full ${windspeed2Classname(positionWindspeed)} flex flex-col items-center justify-center shadow-lg`}>
                    ...
                    <TooltipProvider>
                        ...
                        <TooltipTrigger asChild>
                            ...
                            className={`w-8 h-8 sm:w-10 sm:h-10 p-1 ${(showOrganisations) ? ("bg-blue-600") : ("bg-white")} text-blue-700 rounded-full shadow transition flex items-center justify-center noselect`}
                            >
                                {showOrganisations 
                                ? <UserGroupIcon className="noselect w-6 h-6 text-white" style={{ filter: "drop-shadow(1px 1px 2px rgba(0,0,0,0.5))" }}/>
                                : <UserGroupIcon className="noselect w-6 h-6 text-gray-400" />
                                }
                            </button>
                        </TooltipTrigger>
                        <TooltipContent side="left" sideOffset={20} className="font-light text-sm bg-white text-black border shadow px-3 py-1 rounded-md hidden sm:block">
                    ...
                    <TooltipProvider>
                        ...
                        <TooltipTrigger asChild>
                            ...
                            className={`w-8 h-8 sm:w-10 sm:h-10 p-1 ${(showViewshed) ? ("bg-blue-600") : ("bg-white")} text-blue-700 rounded-full shadow transition flex items-center justify-center noselect`}
                            >
                                {showViewshed ? (
                                    <svg 
                                        ...
                                        viewBox="0 0 576 512"
                                        fill="currentColor"
                                        className="noselect w-5 h-5 text-white"
                                        ...
                                        viewBox="0 0 576 512"
                                        fill="currentColor"
                                        className="noselect w-5 h-5 text-gray-400"
                                        >
                                        <path d="M128 32l32 0c17.7 0 32 14.3 32 32l0 32L96 96l0-32c0-17.7 14.3-32 32-32zm64 96l0 320c0 17.7-14.3 32-32 32L32 480c-17.7 0-32-14.3-32-32l0-59.1c0-34.6 9.4-68.6 27.2-98.3C40.9 267.8 49.7 242.4 53 216L60.5 156c2-16 15.6-28 31.8-28l99.8 0zm227.8 0c16.1 0 29.8 12 31.8 28L459 216c3.3 26.4 12.1 51.8 25.8 74.6c17.8 29.7 27.2 63.7 27.2 98.3l0 59.1c0 17.7-14.3 32-32 32l-128 0c-17.7 0-32-14.3-32-32l0-320 99.8 0zM320 64c0-17.7 14.3-32 32-32l32 0c17.7 0 32 14.3 32 32l0 32-96 0 0-32zm-32 64l0 160-64 0 0-160 64 0z"/>
                                        </svg>
                                )}
                    ...
                    <TooltipProvider>
                        ...
                        <TooltipTrigger asChild>
                            ...
                            className={`w-8 h-8 sm:w-10 sm:h-10 p-1 ${(layersVisible) ? ("bg-blue-600") : ("bg-white")}  rounded-full shadow transition flex items-center justify-center noselect`}
                            >
                                {layersVisible ? (
                                    <Layers className="noselect w-5 h-5 text-white" strokeWidth={2} style={{ filter: "drop-shadow(1px 1px 2px rgba(0,0,0,0.5))" }}/>
                                ) : (
                                    <Layers className="noselect w-5 h-5 text-gray-400" strokeWidth={1.5}/>
                                )}
                            </button>
                        </TooltipTrigger>
                ...
                {/* Vertical right-hand toolbar */}
                <div className="absolute right-4 sm:right-4 top-8-3rem sm-top-9-3rem z-40 select-none">
                    <div className="bg-gray-100 rounded-full shadow-lg p-2 sm:p-2 flex flex-col items-center space-y-2 sm:space-y-2">

                    <TooltipProvider>
                        <Tooltip>
                        <TooltipTrigger asChild>
                            <button type="button" onClick={(e) => mapZoomIn(e)} className="w-8 h-8 sm:w-9 sm:h-9 bg-white text-sm shadow rounded-full active:bg-white focus:outline-none focus:ring-0 noselect">
                             <Plus className={`noselect w-5 h-5 sm:w-7 sm:h-7 ${(currentZoom >= maxZoom) ? "text-gray-200" : "text-black"} translate-x-1-5 sm-translate-x-1`} strokeWidth={2}/>
                            </button>
                        </TooltipTrigger>
                        <TooltipContent side="right" sideOffset={20} className="font-light text-sm bg-white text-black border shadow px-3 py-1 rounded-md hidden sm:block">
                            Zoom into map
                    ...
                    <TooltipProvider>
                        <Tooltip>
                        <TooltipTrigger asChild>
                            <button onClick={mapZoomOut} className="w-8 h-8 sm:w-9 sm:h-9 bg-white text-sm shadow rounded-full active:bg-white focus:outline-none focus:ring-0 noselect">
                             <Minus className={`noselect w-5 h-5 sm:w-7 sm:h-7 ${(currentZoom <= minZoom) ? "text-gray-200" : "text-black"} translate-x-1-5 sm-translate-x-1`} strokeWidth={2}/>
                            </button>
                        </TooltipTrigger>
                        <TooltipContent side="right" sideOffset={20} className="font-light text-sm bg-white text-black border shadow px-3 py-1 rounded-md hidden sm:block">
                            Zoom out of map
                    ...
                    <TooltipProvider>
                        <Tooltip>
                        <TooltipTrigger asChild>
                            <button onClick={mapCentreOnTurbine} className="w-8 h-8 sm:w-9 sm:h-9 rounded-full flex bg-white shadow items-center justify-center noselect">
                            <img
                                alt="Wind turbine"
                                src={`${assetPrefix}/icons/windturbine_black.png`}
                                className="block w-4 h-5 sm:w-5 sm:h-6 translate-x-0.2 noselect"
                            />
                            </button>
                        </TooltipTrigger>
                        <TooltipContent side="right" sideOffset={20} className="font-light text-sm bg-white text-black border shadow px-3 py-1 rounded-md hidden sm:block">
                ...
                <div className="absolute bottom-8rem sm:bottom-12 inset-x-0 flex justify-center px-4 z-30 pointer-events-none">
                    <div
                        className="
                    ...
                    >
                        ...
                        className="text-white text-sm hover:text-gray-300 bg-transparent pointer-events-auto"
                        aria-label="Dismiss"
                        >
                        ×
                        </button>
                ...
                <div className="absolute top-14 sm:top-16 left-0 right-0 px-4 sm:px-0 z-40 pointer-events-none ">
                ...
                <div className="absolute top-5-5rem sm-top-6-5rem left-0 right-0 px-4 sm:px-0 z-30 pointer-events-none ">
                    <div className="relative w-full sm:w-md max-w-md mx-auto">
                        <TooltipProvider>
                            <Tooltip>
                            <TooltipTrigger asChild>
                ...
                <div id="map-container" className="relative w-full" style={{ height: 'calc(var(--vh, 1vh) * 100)' }}>
                    <Map
                        ...
                        <Marker map={mapRef.current.getMap()} onDragStart={onTurbineMarkerDragStart} onDragEnd={onTurbineMarkerDragEnd} longitude={turbinePosition.longitude} latitude={turbinePosition.latitude} draggable={true} anchor="bottom" offset={[0, 0]}>
            ...
            <div className="fixed bottom-0 left-0 w-full h-1/3 overflow-y-auto bg-white sm:bg-white/95 shadow-lg border-t z-50 flex flex-col justify-between px-2 pt-1 pb-2 sm:px-10 sm:pt-0 sm:pb-6">
                ...
                <div className="max-w-screen-xl mx-auto h-full flex flex-col justify-start px-0 sm:px-4 pb-4 select-text">

                    {/* Close button */}
                    <button
                    onClick={closePanel}
                    className="absolute top-1 right-4 text-gray-500 hover:text-gray-700 text-2xl leading-none bg-white"
                    ...
                    <div className="mt-2 sm:mt-6 sm:max-w-[800px] mx-auto">
                        <div className="flex flex-col sm:flex-row sm:items-start mx-2 sm:mr-4">
                            
                            {(organisation.logo_url !== '') &&
                            <div className="w-full sm:w-1/2 mb-4 sm:mb-0 mr-4">
                                <img
                                src={organisation.logo_url}
                                alt={organisation.name}
                                className={`${organisation.logo_transparent && "bg-gray-300"} w-full h:44 min-w-60 sm:w-80 max-h-40 object-contain p-4 m-0`}
                                />
                            </div>
                            }

                            <p className="text-xs text-gray-700 mb-8 sm:mb-4" dangerouslySetInnerHTML={{ __html: organisation.description }}>
                            </p>
                        </div>
                    </div>
        ...
        {/* Voting panel */}
        {turbineAdded && (
        <section style={{ height: 'calc(var(--vh, 1vh) * 36)', minHeight: '16.5rem' }} className="w-full z-50 bg-white sm:bg-white/95 shadow-lg border-t px-4 py-2 sm:py-0 sm:fixed sm:bottom-0 sm:left-0 sm:h-1/3 mobile-h-1-3-plus-48 overflow-y-auto fixed bottom-0 left-0">
            ...
            >
                ...
                <div className="flex mt-0 sm:mt-0 mb-2">
                    <div className="flex-shrink-0 w-25 h-25 sm:w-60 sm:h-40">
                        <div className="hidden sm:block mt-9 ml-5 sm:mt-0 sm:ml-10 relative inline-block">

                        <img
                            src={`${assetPrefix}/icons/check-mark.svg`}
                            alt="Vote"
                            className="hidden sm:block sm:w-48 sm:h-48 object-contain"
                        />
                        ...
                        {(votes !== null) && (
                            <div className="absolute bottom-0 text-right w-11rem">
                                {(votes.confirmed !== 0) &&
                                <TooltipProvider>
                                    <Tooltip>
                                    <TooltipTrigger asChild>
                    ...
                    <div className="ml-0 sm:ml-0 flex flex-col justify-start mt-0 sm:mt-1">
                        ...
                        <div className="hidden sm:flex flex-wrap items-center space-y-0 mt-2 sm:mt-4 select-text">
                        ...
                        <div className="sm:hidden flex flex-wrap gap-1 mt-2 sm:mt-4 select-text">
                ...
                <div className="mt-2 flex flex-row justify-end space-x-3">
                    <Button type="button" onClick={closePanel} variant="default" className="flex-1 bg-gray-200 text-gray-800 px-4 py-2 shadow:none rounded hover:bg-gray-300">
                        Cancel
                    </Button>
                    ...
                    >
                        <div className="w-4 h-4 mr-1 sm:mr-2 flex items-center justify-center">
                            <img
